...
function makeDebugInfo(spec, found, selectedTarim, selectedRival){
  const isImportant =
    spec.label==="1 kg Tam Yağlı Beyaz Peynir" ||
    spec.label==="Sarıyer Kola 2.5 L" ||
    spec.label==="1 kg Çekirdeksiz Karpuz";

  if(!isImportant) return null;

  const tarimCandidates = found.filter(x=>x.marketType==="tarim").map(debugCandidate);
  const rivalCandidates = found.filter(x=>x.marketType==="rival").map(debugCandidate);
  return {
    product: spec.label,
    keywords: spec.keywords||[],
    must: spec.must||[],
    acceptAny: spec.acceptAny||[],
    ban: spec.ban||[],
    selectedTarim: selectedTarim ? debugCandidate(selectedTarim) : null,
    selectedRival: selectedRival ? debugCandidate(selectedRival) : null,
    tarimCandidates,
    rivalCandidates,
    allCandidates: found.slice(0,40).map(debugCandidate)
  };
}
  ...
  try{
    ...
    for(const spec of activeProducts){
      ...
      try{
        ...
        item.alternatives=found.slice(0,20);
        item.tarim=bestTarimOf(found.filter(x=>x.marketType==="tarim"),spec);
        item.rival=bestOf(found.filter(x=>x.marketType==="rival"));
        item.best=bestOf(found);
        item.debugInfo=makeDebugInfo(spec, found, item.tarim, item.rival);
